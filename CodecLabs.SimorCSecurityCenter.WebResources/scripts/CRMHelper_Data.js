var CRMHelper = (function (helper) {
    var WebApi = {
        ...
        Disassociate: function (logicalName, guid, lookupField, flagAsync, successCallback, failCallback) {
            var entityName = Internal.getPluralName(logicalName) + '(' + guid.replace('{', '').replace('}', '') + ')';
            var deleteLookup = '/' + lookupField + '/$ref';

            flagAsync = flagAsync !== false;
            var oDataURL = Internal.createURL(entityName + deleteLookup);

            var flagOk;

            var req = Internal.createRequest("DELETE", oDataURL, flagAsync);
        ...
        Associate: function (parentLogicalName, parentId, childLogicalName, childId, relationship, flagAsync, successCallback, failCallback) {
            var parentEntityUrl = Internal.getPluralName(parentLogicalName) + '(' + parentId.replace('{', '').replace('}', '') + ')';
            var childEntityUrl = Internal.getPluralName(childLogicalName) + '(' + childId.replace('{', '').replace('}', '') + ')';
            relationship = '/' + relationship + '/$ref';

            var association = {
                "@odata.id": Internal.createURL(parentEntityUrl)
            };

            flagAsync = flagAsync !== false;
            var oDataURL = Internal.createURL(childEntityUrl);
            oDataURL += relationship;

            var flagOk;

            var req = Internal.createRequest("POST", oDataURL, flagAsync);
